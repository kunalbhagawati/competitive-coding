...
class Solution:
    ...
    def kClosest(self, points: list[tuple[int, int]], k: int) -> list[tuple[int, int]]:
        _heap = []
        for i in points:
            neg_d_to_origin = -(self._distance(i, (0, 0)))

            if len(_heap) < k:
                heapq.heappush(_heap, (neg_d_to_origin, i))
            elif neg_d_to_origin > _heap[0][0]:
                heapq.heappushpop(_heap, (neg_d_to_origin, i))

        return [i[1] for i in heapq.nsmallest(k, _heap)]
